const App = {
  ...
  check_for_swap_interval_ms: 1000,
...
App.qrCode = (args) => {
  ...
  const link = `bitcoin:${args.address}?amount=${args.amount}`;

  return kjua({back, rounded, size, text: link});
};
...
App.showInvoice = args => {
  ...
  setTimeout(() => {
    details.find('.loading-fee').prop('hidden', true);
    details.find('.fee-loaded').prop('hidden', false);
    details.find('.fee-amount').text(App.format({tokens: Math.round(invoice.tokens * 0.015)}));
  },
  500);
...
App.submitCreateSwapQuote = function(event) {
  ...
  swap.closest('.start-swap').prop('hidden', true);
  ...
  (err, details) => {
    ...
    swap.find('.loading-fee').prop('hidden', false);
    swap.find('.fee-loaded').prop('hidden', true);
    ...
    const qr = App.qrCode({address: swapAddress, amount: swapAmount});

    quote.find('.pay-qr').append($(qr));
...
App.updatedSwapDetails = ({swap}) => {
  ...
  const isReady = !!hasInvoiceDetails;
